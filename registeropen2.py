...
from Isure.forms import OpenEnrollForm2,EnrollForm
...
def registeropen2(request):
	 ...
	 if "guestmeals2" in request.POST:
      ...
      if request.POST: #Posted
         ...
         print "*****Email Entered *********",request.POST['youremail']
         try:
	 ...
	 try:
	   ...
	   print "Lat Long == ",Geo.lat,Geo.lng
         ...
         if this_viewtype == "0":
           print "**************View Type is 0 "
           form = OpenEnrollForm2(data=request.POST)
           if form.is_valid():
              print "*******The form is valid *********"
              request.session["this_entrantEmail"] = request.POST['youremail']
              this_event=request.session["this_eventid"]
              this_email  = request.POST['youremail']
              thisEvent = Event.objects.get(id=int(this_event))
              thatEvent = Event.objects.get(id = thisEvent.opentie.id)
              try:
                print "**************Trying for attendee"
                attendee = Attendees.objects.get(event=thatEvent,email=this_email)
		...
		registered = True           
                ...
                errorlist = []
                errorlist.append("An expected Attendee was not found in database ")
                errorlist.append("The Webmaster has been notified  ")
                request.session['errorlist'] = errorlist
                return redirect('OOps')
              ...
              print "*******The form is NOT Valid *******",form.errors
              ...
              if form.is_valid():
                 ...
                 try:
		        print "**************Trying for Attendee"
		        update_attendee = Attendees.objects.get(event=thatEvent,email=this_email)
                        buildUpdate(request,update_attendee)
                        update_attendee.save()
                        print "***********After Attemdee Save {} ".format(update_attendee.last_name)
		        this_enrollee = Enrollee()
         ...
         return render(request,'registeropen2.html', {'form':form,'viewtype':viewtype,'event':event ,'this_loc':this_loc,'this_enrollee':this_enrollee,'registered':registered,'this_event':this_event,'Geo':Geo} ) 
